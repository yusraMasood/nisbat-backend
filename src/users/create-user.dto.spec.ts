...
describe('CreateUserDto', () => {
	...
	describe('password validation', () => {
		...
		it('should fail with password without uppercase letter', async () => {
			...
			const passwordErrors = errors.filter(error => error.property === 'password');
			expect(passwordErrors).toHaveLength(1);
			expect(passwordErrors[0].constraints?.matches).toContain('uppercase letter');
		});
		...
		it('should fail with password without special character', async () => {
			...
			const passwordErrors = errors.filter(error => error.property === 'password');
			expect(passwordErrors).toHaveLength(1);
			expect(passwordErrors[0].constraints?.matches).toContain('special character');
		});
	});
	...
	describe('userRole validation', () => {
		it('should pass with valid FAMILY role', async () => {
			...
			const errors = await validate(dto);
			const roleErrors = errors.filter(error => error.property === 'userRole');
